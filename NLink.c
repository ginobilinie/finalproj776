...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for ( row = 0; row < height; ++row)
    {
        for ( col = 0; col < width; ++ col)
        {
            ...
            if ( row > 0 ) // up
            {
                ...
                result += colorDistance(R[idx_center], G[idx_center], B[idx_center], R[idx_neighbor], G[idx_neighbor], B[idx_neighbor]);
                edges ++;
            }
	...
	for (idx_center = 0; idx_center < height * width; idx_center++)
	{
		...
		{
			idx_neighbor = ( row ) + ( col - 1 ) * height;
            pairwise[sparse_count] = a = 50 * exp(-beta * colorDistance(R[idx_center], G[idx_center], B[idx_center], R[idx_neighbor], G[idx_neighbor], B[idx_neighbor]));
