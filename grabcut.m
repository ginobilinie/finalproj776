...
[component(ukwn_idx), fore_gmm]=kCluster(pixels(ukwn_idx, :), K);
[component(back_idx), back_gmm]=kCluster(pixels(back_idx, :), K);

% build smooth term for graph cut
pairwise=NLink(pixels, height, width);

iter=1;
num_pixel_change=rect(3) * rect(4);
while num_pixel_change > 150%the convergence criteria
